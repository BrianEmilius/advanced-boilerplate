...
Users.prototype.create = function(req, res, cFunction) {
	...
	if (this.validate.formToken(req.session.token, req.body._once)) {
		...
		// validate reCaptcha
		if (req.body['g-recaptcha-response'] === undefined || req.body['g-recaptcha-response'] === '' || req.body['g-recaptcha-response'] === null || req.body['g-recaptcha-response'].length === 0) {
		...
		// validate username
		if (/[a-zæøåäöü0-9\-_ ]+$/i.test(req.body.username)) user.username = req.body.username
		else {
		...
		// validate password
		if (req.body.password.length > 0 && this.validate.inputMatch(req.body.password, req.body.passwordRepeat)) user.password = this.bcrypt.hashSync(req.body.password, this.bcrypt.genSaltSync(12))
		else {
		...
		if (error) cFunction(false)

		// create user and redirect to root
		else {
			user.createdAt = new Date()
			var temp = new this.users(user)
